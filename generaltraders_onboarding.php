...
if(!empty($_POST))
{
	if(isset($_POST['clientboardingform'])=='clientboardingform')
	{
		...
		if($Agreementcopy !='')
		{
			...
			if(in_array($extension, $allowedExts))
			{
				...
				$agreementcopynewfilename = 'gt_agrcopy_'.$accountId.'_'.date("dmyhis").'_'.rand().'.'.end($temp);
	...
	if(!empty(array_filter($_FILES['kyc']['name'])))
		...
		foreach ($_FILES['kyc']['tmp_name'] as $key => $value)
			 {
						...
						if(in_array($extension, $allowedExts))
						{
							...
							$kycfilename = 'gt_kyc_'.$accountId.'_'.date("dmyhis").'_'.rand().'.'.end($temp);
	 ...
	 ////////////////// cancel cheque ////////////
		...
		if($cancelcheq !='')
		{
			...
			if(in_array($extension, $allowedExts))
			{
				...
				$cancelcheqnewfilename = 'gt_calche_'.$accountId.'_'.date("dmyhis").'_'.rand().'.'.end($temp);
	 ...
	 header("Location:generaltraders_onboarding.php?act=1");
...
<?php require_once('header.php'); ?>
  <title>GeneralTraders Onboarding form</title>
  <style>
...
<body>
  ...
  <div class="wrapper">
    ...
    <div id="content">
      ...
      <br><br>
      <h2>GeneralTraders Onboarding Form</h2>
	   <button class="btn btn-dark"><a href="view_generaltraders.php">View GeneralTraders</a></button>
