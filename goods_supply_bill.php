...
if(!empty($_POST))
{
	...
	$uploadbill=isset($_FILES['uploadbill']['name']) ? $_FILES['uploadbill']['name'] : NULL;
		...
		if($uploadbill !='')
		{
			...
			if(in_array($extension, $allowedExts))
			{
				...
				$newfilename = 'gsb'.date("dmyhis").'_'.rand().'.'.end($temp);
	...
	$lastid = $account->setGoods_SupplyBill($accountId,$collection_center,$clientlist,$names_list,$billnumber,$newfilename,$transportation,$otherExpenses,$total);
	...
	foreach ($items as $itemId)
	 {
			...
			$account->setGSB_items($insertedid,$itemId, $quantity, $price);
	}
	///////////////php mail///////////
  ...
  // $mail->addAddress('swjnambati@gmail.com', 'Hssagri'); // Add a recipient
   ...
   // $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';
    ...
    $rejectLink = 'https://hssagri.in/handle-approval-gsb.php?action=reject&user_id='.$insertedid.'';
					...
					<p><b>Below are the details from Goods Supply Bill. </b></p>
					
					Clients Name : ".$clientsname."<br/>
					Item Code : ".$item_codes."<br/>
